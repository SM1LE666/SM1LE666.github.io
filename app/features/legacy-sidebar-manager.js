(function () {
  ...
  class LegacySidebarManager {
    ...
    getFilteredMatches() {
      ...
      return (this.currentMatches || []).filter(
        (m) =>
          LegacySidebarManager.normalizeMapKey(m.map) === this.currentMapFilter,
      );
    }
      ...
      try {
        ...
        if (window.FaceitAPI && window.FaceitAPI.getAllMapsStats) {
          ...
          allMapsStats.forEach((map) => {
            const key = LegacySidebarManager.normalizeMapKey(map.name);
        ...
        try {
          ...
          if (!this.availableMapOptions.length) {
            ...
            (this.currentMatches || []).forEach((m) => {
              const raw = m.map || "";
              const key = LegacySidebarManager.normalizeMapKey(raw) || "";
              if (!key) return;
  ...
  window.LegacySidebarManager = LegacySidebarManager;
  if (!window.SidebarManager) {
    window.SidebarManager = LegacySidebarManager;
  }
})();
